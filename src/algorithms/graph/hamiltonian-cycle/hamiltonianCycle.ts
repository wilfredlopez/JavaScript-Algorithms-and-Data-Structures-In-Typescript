 ...
 */
function isSafe(
  adjacencyMatrix: Array<number[]>,
  verticesIndices: ObjectOf<number>,
  cycle: GraphVertex[],
  vertexCandidate: GraphVertex,
) {
  ...
  const candidateDuplicate = cycle.find((vertex) =>
    vertex.getKey() === vertexCandidate.getKey()
  );
...
interface hamiltonianCycleArgs {
  adjacencyMatrix: Array<number[]>;
  vertices: GraphVertex[];
  verticesIndices: ObjectOf<number>;
  cycles: Array<GraphVertex[]>;
  cycle: GraphVertex[];
}
...
function hamiltonianCycleRecursive({
  ...
  cycles,
  cycle,
}: hamiltonianCycleArgs) {
  // Clone cycle in order to prevent it from modification by other DFS branches.
  const currentCycle = [...cycle].map((vertex) =>
    new GraphVertex(vertex.value)
  );
 ...
 * @return {GraphVertex[][]}
 */
export default function hamiltonianCycle(graph: Graph): Array<GraphVertex[]> {
