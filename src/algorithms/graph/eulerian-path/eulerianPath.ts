...
export default function eulerianPath(graph: Graph) {
  ...
  const evenRankVertices: ObjectOf<GraphVertex> = {};

  // Set that contains all vertices with odd rank (number of neighbors).
  const oddRankVertices: ObjectOf<GraphVertex> = {};

  // Set of all not visited edges.
  const notVisitedEdges: ObjectOf<GraphEdge> = {};
  ...
  let startVertex: GraphVertex | null = null;
  ...
  while (Object.values(notVisitedEdges).length) {
    ...
    } else if (currentVertex.getKey() === edgeToDelete.endVertex.getKey()) {
