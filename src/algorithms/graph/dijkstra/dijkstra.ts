...
export default function dijkstra(graph: Graph, startVertex: GraphVertex) {
  ...
  const distances: ObjectOf<number> = {};
  const visitedVertices: ObjectOf<GraphVertex> = {};
  const previousVertices: ObjectOf<GraphVertex | null> = {};
  const queue = new PriorityQueue<GraphVertex>();
