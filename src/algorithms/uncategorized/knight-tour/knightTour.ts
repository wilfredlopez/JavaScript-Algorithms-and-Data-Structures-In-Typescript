 ...
 * @return {number[][]}
 */
function getPossibleMoves(chessboard: Array<number[]>, position: number[]) {
  ...
  return possibleMoves.filter((move) => {
    ...
    return move[0] >= 0 && move[1] >= 0 && move[0] < boardSize &&
      move[1] < boardSize;
  });
}
 ...
 * @return {boolean}
 */
function isMoveAllowed(chessboard: Array<number[]>, move: number[]) {
 ...
 * @return {boolean}
 */
function isBoardCompletelyVisited(
  chessboard: Array<number[]>,
  moves: Array<number[]>,
) {
 ...
 * @return {number[][]}
 */
export default function knightTour(chessboardSize: number) {
  // Init chessboard.
  const chessboard = Array(chessboardSize).fill(null).map(() =>
