...
// the less value for priority example 0. the greater the priority is.
export default class PriorityQueue<T extends any> extends MinHeap<T> {
  ...
  comparePriority(a: T, b: T) {
    ...
    const pa = this.priorities.get(a) || -1
    const pb = this.priorities.get(b) || -1
    return pa < pb ? -1 : 1
  }
